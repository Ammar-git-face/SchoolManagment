        ...
        <div className="flex min-h-screen bg-gray-50">
            ...
            <div className="flex-1 md:ml-64 min-h-screen">
                ...
                <div className="md:hidden flex items-center justify-between bg-white px-4 py-3 shadow-sm sticky top-0 z-10">
                    ...
                    <h1 className="font-semibold text-black">Subjects</h1>
                    <div className="w-8" />
                </div>
                ...
                <div className="p-4 md:p-8 max-w-5xl mx-auto">
                    <div className="hidden md:flex items-center justify-between mb-6">
                        <div>
                            <h1 className="text-2xl font-bold text-black">Subject Management</h1>
                            <p className="text-sm text-gray-400 mt-1">Define subjects per class — used for CA and Exam entry</p>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => setBulkModal(true)}
                                className="px-4 py-2 border border-blue-600 text-blue-200 rounded-xl text-sm font-medium hover:bg-blue-50 transition">
                                Bulk Add
                            </button>
                    ...
                    <div className="flex gap-2 mb-4 md:hidden">
                        <button onClick={() => setBulkModal(true)} className="flex-1 py-2 border border-blue-600 text-blue-200 rounded-xl text-sm font-medium">Bulk Add</button>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <p className="text-black font-medium">No subjects yet</p>
                            ...
                            {Object.keys(grouped).sort().map(className => (
                                <div key={className} className="bg-white rounded-xl shadow overflow-hidden">
                                    <div className="px-5 py-3 bg-gray-50 border-b flex items-center justify-between">
                                        <h3 className="font-semibold text-black">{className}</h3>
                                        ...
                                        {grouped[className].map(s => (
                                            <div key={s._id} className="px-5 py-3 flex items-center justify-between gap-3">
                                                <div className="flex-1">
                                                    <div className="font-medium text-black text-sm">{s.name}</div>
                                                ...
                                                <div className="flex gap-2 shrink-0">
                                                    <button onClick={() => setEditModal({ ...s })}
                                                        className="px-3 py-1 text-xs bg-gray-100 text-black rounded-lg hover:bg-gray-200 transition">Edit</button>
                ...
                <div className="fixed inset-0   bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-md p-6">
                        ...
                        <div className="space-y-3">
                            <div>
                                <label className="text-xs text-black mb-1 block">Subject Name</label>
                                <input value={newSubject.name} onChange={e => setNewSubject({ ...newSubject, name: e.target.value })}
                                    placeholder="e.g. English Language"
                                    className="w-full border rounded-xl px-3 py-2.5 text-sm text-black focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                ...
                <div className="fixed inset-0   bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-md p-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="bg-gray-50 rounded-xl p-3 text-xs text-black">
                                Class: <span className="font-medium text-black">{editModal.className}</span> (cannot change)
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs text-black mb-1 block">Max CA</label>
                                    <input type="number" value={editModal.maxCA}
                                        onChange={e => setEditModal({ ...editModal, maxCA: Number(e.target.value) })}
                                        className="w-full border rounded-xl px-3 py-2.5 text-sm text-black focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label className="text-xs text-black mb-1 block">Max Exam</label>
                        ...
                        <div className="flex gap-3 mt-5">
                            <button onClick={() => setEditModal(null)} className="flex-1 py-2.5 bg-gray-100 text-black rounded-xl text-sm">Cancel</button>
                ...
                <div className="fixed inset-0   bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-md p-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs text-black mb-1 block">Max CA (all)</label>
                                    <input type="number" value={bulkMaxCA} onChange={e => setBulkMaxCA(Number(e.target.value))}
                                        className="w-full border rounded-xl px-3 py-2.5 text-sm text-black focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label className="text-xs text-black mb-1 block">Max Exam (all)</label>
                                    <input type="number" value={bulkMaxExam} onChange={e => setBulkMaxExam(Number(e.target.value))}
                                        className="w-full border rounded-xl px-3 py-2.5 text-sm text-black focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs text-black mb-1 block">Subjects (one per line)</label>
                                <textarea value={bulkText} onChange={e => setBulkText(e.target.value)}
                                    rows={8} placeholder={"English Language\nMathematics\nBasic Science\nSocial Studies"}
                                    className="w-full border rounded-xl px-3 py-2.5 text-sm text-black focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono" />
                            </div>
                        </div>
                        <div className="flex gap-3 mt-5">
                            <button onClick={() => { setBulkModal(false); setBulkText('') }}
                                className="flex-1 py-2.5 bg-gray-100 text-black rounded-xl text-sm">Cancel</button>
