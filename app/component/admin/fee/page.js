...
const downloadReceipt = (record, type) => {
    ...
    const lines = [
        ...
        isTeacher ? `Month:       ${record.month} ${record.year}` : `Class:       ${record.studentClass || "—"}`,
        isTeacher ? `Basic Salary: ₦${Number(record.basicSalary || 0).toLocaleString()}` : `Term:        ${record.term}`,
        isTeacher ? `Net Pay:     ₦${Number(record.netPay || 0).toLocaleString()}` : `Amount:      ₦${Number(record.amount || 0).toLocaleString()}`,
        `Status:      ${record.status?.toUpperCase()}`,
        record.paidAt ? `Paid On:     ${new Date(record.paidAt).toLocaleDateString("en-GB")}` : "",
        record.flwRef ? `Reference:   ${record.flwRef}` : record.txRef ? `TX Ref:      ${record.txRef}` : "",
        "----------------------------------------",
...
const Badge = ({ status }) => {
    const cfg = { paid: "bg-green-100 text-green-700", pending: "bg-yellow-100 text-yellow-700", unpaid: "bg-red-100 text-red-700" }
    return <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${cfg[status] || cfg.pending}`}>{status}</span>
}

const FeesContent = () => {                                                   // ✅ ADDED: renamed from AdminFees to FeesContent
    ...
    useEffect(() => {
        const load = async () => {
            ...
            } catch (err) { console.log("Fee load error:", err.message) }
    ...
    const filtered   = activeData.filter(r => {
        ...
        return name?.toLowerCase().includes(search.toLowerCase()) && (statusFilter === "all" || r.status === statusFilter)
    })
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="fixed top-0 left-0 right-0 md:ml-64 bg-white border-b border-gray-200 px-4 py-3 z-10 shadow-sm flex items-center gap-3">
                <button onClick={() => setSidebarOpen(true)} className="md:hidden p-2 rounded-lg hover:bg-gray-100 text-black flex-shrink-0">
                    <Menu size={20} />
                </button>
                <div>
            ...
            <div className="md:ml-64 pt-20 px-4 pb-10">
                    ...
                    {[
                        ...
                        <button key={t.key} onClick={() => { setTab(t.key); setSearch(""); setStatusFilter("all") }}
                            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium transition
                                ${tab === t.key ? "bg-blue-200 text-black shadow-sm" : "bg-white text-black border border-gray-200 hover:bg-gray-50"}`}>
                            {t.icon}{t.label}
                        </button>
                    ))}
                </div>
                    ...
                    {cards.map((c, i) => (
                        <div key={i} className={`rounded-xl p-4 shadow-sm border border-gray-100 ${c.color}`}>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <div className={`p-2 rounded-lg ${c.icon}`}><DollarSign size={16} /></div>
                            </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-4">
                    <div className="relative flex-1 max-w-xs">
                        ...
                        <input type="text" placeholder={`Search ${tab === "students" ? "students" : "teachers"}...`}
                            ...
                            className="w-full pl-8 pr-3 py-2 border border-gray-200 rounded-xl text-xs text-black focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white" />
                    </div>
                    ...
                    <table className="w-full min-w-[600px]">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-100">
                                {headers.map(h => <th key={h} className="px-4 py-3 text-left text-xs text-black font-medium">{h}</th>)}
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                ...
                                filtered.map((f, i) => (
                                    <tr key={f._id || i} className="border-t border-gray-100 hover:bg-gray-50 text-xs">
                                        ...
                                        <td className="px-4 py-3 text-black">{f.paidAt ? new Date(f.paidAt).toLocaleDateString("en-GB") : "—"}</td>
                                        <td className="px-4 py-3">
                                            <button onClick={() => downloadReceipt(f, "student")} disabled={f.status !== "paid"}
                                                className="flex items-center gap-1 text-blue-200 hover:text-blue-400 disabled:text-gray-300 disabled:cursor-not-allowed transition">
                                                <Download size={13} /><span>Receipt</span>
                                            </button>
                                        </td>
                                    </tr>
                                ))
                                ...
                                filtered.map((p, i) => (
                                    <tr key={p._id || i} className="border-t border-gray-100 hover:bg-gray-50 text-xs">
                                        ...
                                        <td className="px-4 py-3 text-black">{p.paidAt ? new Date(p.paidAt).toLocaleDateString("en-GB") : "—"}</td>
                                        <td className="px-4 py-3">
                                            <button onClick={() => downloadReceipt(p, "teacher")} disabled={p.status !== "paid"}
                                                className="flex items-center gap-1 text-blue-200 hover:text-blue-400 disabled:text-gray-300 disabled:cursor-not-allowed transition">
                                                <Download size={13} /><span>Receipt</span>
                                            </button>
                                        </td>
                ...
                {!loading && <p className="text-xs text-gray-400 mt-3">Showing {filtered.length} of {activeData.length} records</p>}
            </div>
        </div>
    )
}

function AdminFees() {                                                        // ✅ ADDED: new default export wraps content in gate
    return (                                                                  // ✅ ADDED
        <PremiumGate feature="Finance & Transactions">                        // ✅ ADDED
            <FeesContent />                                                   // ✅ ADDED
        </PremiumGate>                                                        // ✅ ADDED
    )                                                                         // ✅ ADDED
}                                                                             // ✅ ADDED
