...
const ParentPTA = () => {
    ...
    return (
        <div>
            ...
            <div className="mx-4 md:ml-64 -mt-6">
                {displayed.length === 0 ? (
                            ...
                            <div key={m._id} className="shadow-xl hover:shadow-2xl rounded-xl p-4 bg-white">
                                <nav className="flex justify-between items-center mb-3">
                                    <span className="flex items-center text-sm font-bold gap-1">
                                        {m.type === 'video' ? <VideoIcon size={15} className="text-orange-500" /> : <Mic size={15} className="text-orange-500" />}
                                        {m.title}
                                    </span>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${m.status === 'upcoming' ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'}`}>
                                        {m.status}
                                    </span>
                                </nav>
                                ...
                                {m.status === 'upcoming' && (
                                    <button onClick={() => setActiveCall(m)}
                                        className="w-full flex items-center justify-center gap-1 text-xs bg-orange-500 text-white py-1.5 rounded-xl hover:bg-orange-600">
                                        {m.type === 'video' ? <VideoIcon size={12} /> : <Mic size={12} />} Join Meeting
                                    </button>
                                )}
                            </div>
                        ))}
                ...
                <div className="fixed inset-0 bg-black z-50 flex flex-col">
                    <div className="flex items-center justify-between px-4 py-2 bg-gray-900">
                        <div className="flex items-center gap-2">
                            {activeCall.type === 'video' ? <VideoIcon size={16} className="text-orange-400" /> : <Mic size={16} className="text-orange-400" />}
                        ...
                        <button onClick={() => setActiveCall(null)}
                            ...
                            <X size={12} /> Leave Meeting
                        </button>
                    </div>
                    <iframe
                        src={`https://meet.jit.si/${activeCall.roomName}#userInfo.displayName="${encodeURIComponent(user?.name || 'Parent')}"`}
