...
const gradeColor = (g) =>
    ...
    : g?.startsWith("B") ? "text-blue-500 bg-blue-100"
    : g?.startsWith("C") ? "text-yellow-500 bg-yellow-100"
    : "text-red-500 bg-red-100"

const scoreColor = (val, max) => {
    const pct = (val / max) * 100
    return pct >= 80 ? "text-green-500" : pct >= 60 ? "text-blue-500" : pct >= 40 ? "text-yellow-500" : "text-red-500"
}
        ...
        <div className="flex min-h-screen bg-gray-50">
            ...
            <div className="flex-1 md:ml-64 min-h-screen">
                ...
                <div className="md:px-6 px-4 pt-8 pb-10">
                    <div className="mb-6">
                        <h1 className="text-2xl font-bold text-gray-800">My Child's Results</h1>
                    ...
                    {children.length === 0 ? (
                        ...
                        <div className="flex items-center gap-3 mb-6 flex-wrap">
                            <select className="border border-gray-200 rounded-xl px-4 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-x-auto mb-5">
                        <div className="px-6 py-4 border-b border-gray-100">
                            <h2 className="text-sm font-semibold text-gray-700">Results — {selectedTerm} · {selectedSession}</h2>
                        </div>
                                ...
                                {loading ? (
                                    ...
                                    return (
                                        <tr key={r._id} className="border-t border-gray-100 hover:bg-gray-50">
                                            <td className="px-6 py-4 text-sm text-gray-700">{r.subject}</td>
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-500">{remark}</td>
                                        </tr>
                                    )
                                })}
                            </tbody>
                        ...
                        {childResults.length > 0 && childResults[0]?.teacherRemark && (
                            <div className="px-6 py-4 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50 rounded-b-2xl">
                                <div>
                                    <p className="text-xs font-semibold text-gray-500 mb-1">Class Teacher's Remark</p>
                                    <p className="text-sm text-gray-700">{childResults[0].teacherRemark}</p>
                                </div>
                                <div>
                                    <p className="text-xs font-semibold text-gray-500 mb-1">Principal's Remark</p>
                                    <p className="text-sm text-gray-700">{childResults[0].principalRemark}</p>
                                </div>
                            </div>
                        )}
                    </div>
