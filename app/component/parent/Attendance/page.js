        ...
        <div className="flex min-h-screen bg-gray-50">
            ...
            <div className="flex-1 md:ml-64 min-h-screen">
                ...
                <div className="p-4 md:p-8 max-w-4xl mx-auto">
                    <h1 className="hidden md:block text-2xl font-bold text-black mb-6">Attendance Record</h1>
                    ...
                    {children.length === 0 ? (
                        ...
                        <div className="bg-white rounded-xl shadow p-4 md:p-5 mb-5 grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div>
                                <label className="text-xs text-black mb-1 block">Child</label>
                        ...
                        <>
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
                                <div className="bg-white rounded-xl shadow p-4 text-center col-span-2 md:col-span-1">
                                    ...
                                    <div className="text-xs text-black mt-1">Rate</div>
                                </div>
                                {[
                                    { label: 'Total Days', value: summary.total,   color: 'text-black' },
                    ...
                    <div className="bg-white rounded-xl shadow overflow-hidden">
                        <div className="px-5 py-4 border-b"><h2 className="font-semibold text-black">Daily Records</h2></div>
                        {loading ? (
                                            ...
                                            {records.map(r => {
                                                ...
                                                return (
                                                    <tr key={r._id} className="hover:bg-gray-50">
                                                        <td className="px-5 py-3 text-sm text-black">{d.toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                                                        <td className="px-5 py-3 text-sm text-black">{d.toLocaleDateString('en-NG', { weekday: 'long' })}</td>
                                                        <td className="px-5 py-3">
                                                            <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${STATUS_BADGE[r.status]}`}>{r.status}</span>
                                                        </td>
                                                        <td className="px-5 py-3 text-sm text-black">{r.remark || '—'}</td>
                                                    </tr>
                                                )
                                            })}
                                        </tbody>
                                    ...
                                    {records.map(r => {
                                        ...
                                        return (
                                            <div key={r._id} className="px-4 py-3 flex items-center justify-between">
                                                <div>
                                                    <div className="text-sm font-medium text-black">{d.toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
